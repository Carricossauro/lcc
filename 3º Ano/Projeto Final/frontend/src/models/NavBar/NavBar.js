...
export default function NavBar(props) {
  ...
  return (
    <div className="fixed z-50 top-0 left-0 h-16 w-screen bg-color5 flex flex-wrap text-white shadow-md">
      ...
      {props.size < 1024 && (
        <div
          className={`w-56 h-screen flex flex-col bg-color1 justify-start items-center absolute top-16 right-0 shadow-md duration-1000 ${
            expanded ? "translate-x-0" : "translate-x-full"
          }`}
        >
          <div className="flex justify-center my-3">
            <button
              className="flex items-center justify-center h-9 w-32 hover:bg-color4 rounded cursor-pointer duration-1000"
              onClick={() => redirect("/About")}
            >
              ABOUT US
            </button>
          ...
          {!props.loggedIn && (
            <>
              ...
              <div className="flex justify-center my-3">
                <button
                  className="flex items-center justify-center h-9 w-32 bg-color4 rounded cursor-pointer hover:text-color4 hover:font-bold duration-1000"
                  onClick={() => redirect("/Login")}
                >
                  LOGIN
                </button>
