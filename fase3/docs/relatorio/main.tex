...
\begin{itemize}
    \item \textbf{Desenho de superfícies de Bezier}
        \begin{description}
        \item Alteração do \textit{generator} de modo a ler ficheiros com \textit{patches} de Bezier e pontos de controlo e retornar a lista de triangulos necessarios para desenhar a supreficie, de acordo com o nível de tesselação requerido. 
        \end{description}

    \item \textbf{Extensão das translações e rotações}
    \begin{description}
    \item As translações devem poder ser constituidas por um conjunto de pontos de controlo, que definam um curva de Catmul-Rom por onde o modelo sobre o qual é aplicada a transformação vai passar, assim como o número de segundos que este demora a descrever a curva.
