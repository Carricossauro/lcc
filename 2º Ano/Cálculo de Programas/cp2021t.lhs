...
A solução para |outExpAr| pode ser obtida através da igualdade |outExpAr . inExpAr = id|:
\begin{eqnarray*}
\start
  |outExpAr . inExpAr = id|
%
...
%
\just\equiv{ Universal-+; Def-|num_ops|; Fusão-+ }
%
\begin{lcbr}
  |outExpAr . const X = i1|
\\
...
\end{lcbr}
%
\just\equiv{ Universal-+; Def-|ops|; Fusão-+ }
%
\begin{lcbr}
    |outExpAr . const X = i1|
\\
...
\\
    |outExpAr ((uncurry Un (x,y))) = i2 (i2 (i2 (x,y)))|
\end{lcbr}
%
\just\equiv{ Def-|bin|; Def-|uncurry| }
%
\end{eqnarray*}
...
g_eval_exp a = either (const a) num_ops where
              ...
              ops = either bina (uncurry unar)
              bina (Sum, par) = uncurry (+) par
              bina (Product, par) = uncurry (*) par
              unar Negate = negate
              unar E = expd
---
clean (Bin Product (N 0) b) = i2 (i1 0)
clean (Bin Product a (N 0)) = i2 (i1 0)
clean (Bin Product (N 1) b) = clean b
clean (Bin Product a (N 1)) = clean a
clean (Bin Sum (N 0) b) = clean b
...
sd_gen :: Floating a =>
    ...
    -> (ExpAr a, ExpAr a)
sd_gen = either (const (X, N 1)) q where
   q = either constante ops
   constante = split (N . id) (N . (const 0))
   ops = either binar unar
   unar (E,(x1,dx1)) = (Un E x1, Bin Product (Un E x1) dx1)
   unar (Negate, (x1,dx1)) = (Un Negate x1, Un Negate dx1)
   binar (Sum, ((x1,dx1),(x2,dx2))) = (Bin Sum x1 x2, Bin Sum dx1 dx2)
   binar x = (esq x, dir x)
   esq (Product, ((x1,dx1),(x2,dx2))) = Bin Product x1 x2 
   dir (Product, ((x1,dx1),(x2,dx2))) = Bin Sum (Bin Product x1 dx2) (Bin Product dx1 x2)
\end{code}

\begin{code}
ad_gen v = either b q where
   b () = (v, 1)
   q = either constante ops
   constante x = (x,0)
   ops = either binar unar
   binar (Sum,((x1,dx1),(x2,dx2))) = (x1 + x2, dx1 + dx2)
   binar (Product,((x1,dx1),(x2,dx2))) = (x1 * x2, (x1 * dx2) + (dx1 * x2))
   unar (Negate,(x1,dx1)) = (-x1, -dx1)
   unar (E,(x1,dx1)) = (expd x1, dx1 * expd x1)
\end{code}
...
loop = b >< d where
   succ2 = (2+)
   b = split (succ . p1) (mul . (succ >< id))
   d = split (succ2 . p1) (mul . (mul . split succ2 succ >< id))
inic = ((0,1),(0,1))
prj ((s1,facN),(s2,fac2N)) = div fac2N ((facN*(s1+1) * facN))
\end{code}
por forma a que
\begin{code}
...
O valor de n! pode ser obtido pelas funções:
\begin{eqnarray*}
\start
\begin{lcbr}
  |facN 0 = 1|
\\
  |facN (n+1) = s1 (n+1) * facN n|
\\
  |s1 0 = 0|
\\
  |s1 (n+1) = 1 + s1 n|
...
\begin{lcbr}
  |facN . zero = one|
\\
  |facN . succ = mul . (succ >< id) . split s1 facN|
\\
  |s1 . zero = zero|
\\
  |s1 . succ = succ . p1 . split s1 facN|
\end{lcbr}
%
\just\equiv{ Eq-+; Def-in }
%
\begin{lcbr}
  |facN . in = either one (mul . (succ >< id) . split s1 facN)|
\\
  |s1 . in = either zero (succ . p1 . split s1 facN)|
\end{lcbr}
%
\just\equiv{ Absorção-+ }
%
\begin{lcbr}
  |facN . in = either one (mul . (succ >< id)) . (id + split s1 facN)|
\\
  |s1 . in = either zero (succ . p1) . (id + split s1 facN)|
\end{lcbr}
%
\just\equiv{ Fokkinga }
%
  |split s1 facN = cataNat (split (either zero (succ . p1 . split s1 facN)) (either one (mul . (succ >< id) . split s1 facN)))|
%
\just\equiv{ Lei da Troca }
%
  |split s1 facN = cataNat (either (split zero one) (split (succ . p1) (mul . (succ >< id))))|
%
\end{eqnarray*}
Este mesmo processo pode ser usado para obter (2n)!:

(Usaremos uma auxiliar |succ2 = (2+)|)
\begin{eqnarray*}
\start
  |split s2 fac2N = cataNat (either (split zero one) (split (succ2 . p1) (mul . (mul . split succ2 succ >< id))))|
%
\end{eqnarray*}
Temos então:
\begin{eqnarray*}
...
%
\begin{lcbr}
  |split (split s1 facN) (split s2 fac2N) = split (cataNat (either a b)) (cataNat (either a d))|
\\
  |a = split zero one|
\\
  |b = split (succ . p1) (mul . (succ >< id)|
...
%
\begin{lcbr}
  |split (split s1 facN) (split s2 fac2N) = cataNat ((either a b >< either a d) . split (F p1) (F p2))|
\\
  |a = split zero one|
\\
  |b = split (succ . p1) (mul . (succ >< id)|
...
%
\begin{lcbr}
  |split (split s1 facN) (split s2 fac2N) = cataNat (split (either a b . F p1) (either a d . F p2))|
\\
  |a = split zero one|
\\
  |b = split (succ . p1) (mul . (succ >< id)|
...
\end{lcbr}
%
\just\equiv{ Definição do functor de números naturais }
%
\begin{lcbr}
  |split (split s1 facN) (split s2 fac2N) = cataNat (split (either a b . (id + p1)) (either a d . (id + p2)))|
\\
  |a = split zero one|
\\
  |b = split (succ . p1) (mul . (succ >< id)|
...
%
\begin{lcbr}
  |split (split s1 facN) (split s2 fac2N) = cataNat (split (either a (b . p1)) (either a (d . p2)))|
\\
  |a = split zero one|
\\
  |b = split (succ . p1) (mul . (succ >< id)|
...
%
\begin{lcbr}
  |split (split s1 facN) (split s2 fac2N) = cataNat (either (split a a) (split (b . p1) (d . p2)))|
\\
  |a = split zero one|
\\
  |b = split (succ . p1) (mul . (succ >< id)|
...
%
\begin{lcbr}
  |split (split s1 facN) (split s2 fac2N) = for (split (b . p1) (d . p2)) (((0,1),(0,1)))|
\\
  |b = split (succ . p1) (mul . (succ >< id))|
\\
  |d = split (succ2 . p1) (mul . (mul . split succ2 succ >< id))|
...
%
\end{eqnarray*}
Daqui, podemos retirar que |loop = split (b . p1) (d . p2) = b >< d| e |inic = ((0,1),(0,1))|.

\subsection*{Problema 3}

\begin{code}
calcLine :: NPoint -> (NPoint -> OverTime NPoint)
calcLine l l2 d = (p1 . (cataList (g d)) . zip l) l2 where
   g d = either (b d) q
   b d () = ([],d)
   q ((a,b), (l,d)) = (linear1d a b d:l, d)
--
deCasteljau :: [NPoint] -> OverTime NPoint
deCasteljau l t = (hyloAlgForm t alg coalg) l where
   coalg [x] = i2 ((x,x),[])
   coalg [] = i1 ()
   coalg (x:y:[]) = i2 ((x,y), [])
   coalg (x:y:t) = i2 ((x,y),y:t)
   alg t = either nil (algAux t)
   algAux t ((x,y), r) = calcLine x y t:r

hyloAlgForm t alg coalg = (auxCasteljau t) . hyloList (alg t) coalg where
   auxCasteljau a [x] = x
   auxCasteljau a l = (hyloAlgForm a alg coalg) l
\end{code}
...
avgLTree = p1.cataLTree gene where
   gene = either b q
   b = split id (const 1)
   q fs = (top fs / bottom fs, bottom fs)
   top (f1,f2) = uncurry (*) f1 + uncurry (*) f2
   bottom = uncurry (+) . (p2 >< p2)
\end{code}
